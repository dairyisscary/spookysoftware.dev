...
export function Root(props: { children?: JSX.Element; header: JSX.Element }) {
  const headerChildren = children(() => props.header);
  const mainChildren = children(() => props.children);
  return (
    <>
      <a
        class={[
      ...
      <div class="isolate flex min-h-dvh flex-col">
        ...
        <div class="isolate">
          ...
          <div class="sps-main-contain space-y-2 py-12 md:space-y-4">{headerChildren()}</div>
          <Terrain class="relative z-2" height={200} width={1400} />
        </div>

        <div
          id={MAIN_ID}
          class={["flex flex-1 flex-col", Boolean(mainChildren()) && "bg-sps-bg-dark shadow-lg"]}
        >
          <Show when={mainChildren()} fallback={<Rainbow />}>
            {(children) => (
              <>
