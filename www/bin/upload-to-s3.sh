...
aws s3 cp --recursive --metadata-directive REPLACE --content-type "text/html; charset=utf-8" --cache-control "$BROWSER_EXPIRES" --exclude "*" --include "*.html" public "s3://$BUCKET_NAME"
...
aws s3 cp --metadata-directive REPLACE --content-type "application/rss+xml" --cache-control "$BROWSER_EXPIRES" public/rss.xml "s3://$BUCKET_NAME"
